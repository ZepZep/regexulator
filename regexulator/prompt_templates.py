...
# arguments: examples, extractions, cheatsheet
...
# arguments: pattern, examples_tp, extractions_tp, examples_fn, extractions_fn, examples_fp, extractions_fp
IMPROVE = rf"""Improve the following regular expression to better match the following positive examples but not match the negative examples.

## Regular expression to improve 
{{pattern}}

## Positive examples that the expression already matches and should remain to match with added \` \` highlights
{{examples_tp}}
...
## Additional positive examples that the regex should also match with added \` \` highlights:
{{examples_fn}}
...
## Negative examples (do *NOT* match, do not match, don't match) with added \` \` highlights
{{examples_fp}}
...
First, write down how to differentiate between the positive and negative examples and extractions and find common patterns in them. Use them to construct the regex. Mind the order of the patterns.  make sure to match the positive examples but not match the negative ones.
